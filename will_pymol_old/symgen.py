...
class ComponentCenterVisitor(object):
    ...
    def __init__(self, symelems, extranodes=[], label="NODES", colors=[], showlinks=1, **kwargs):
    ...
    def makeCCtree(self, dhint=1.001):
        ...
        while queue:
            ...
            closedist, closest = 9e9, []
            for priCC, CClist in self.priCCtoCClist.items():
                for CC in CClist:
                    if CC in self.parentmap.keys():
                        continue
    ...
    def sanity_check(self):
        ...
        for priCC in self.primaryCCs:
            assert priCC in self.priCCtoCClist.keys()
        for priCC in self.priCCtoCClist.keys():
    ...
    def make_symdef(self, **kwargs):
        ...
        for ip, val in enumerate(self.priCCtoCCframes.items()):
            ...
            for icc, val2 in enumerate(CCframes.items()):
                ...
                CC, STNs = val2
                assert len(STNs) > 0
                PCC = None if not CC in self.parentmap.keys(
