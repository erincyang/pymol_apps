...
def guesscxaxis(sele, nfold=None, chains0=[], extrasel="name CA"):
...
def aligncx(sele, nfold, alignsele=None, tgtaxis=Uz, chains=[], extrasel="name CA"):
    if not alignsele:
        alignsele = sele
...
def align_helix(sele, nrepeat, tgt_axis=Vec(1, 0, 0)):
    ...
    sel1 = "( %s ) and resi %i-%i" % (sele, resi0 +
                                      0 * nrepeat, resi0 + 1 * nrepeat - 1)
    sel2 = "( %s ) and resi %i-%i" % (sele, resi0 +
                                      1 * nrepeat, resi0 + 2 * nrepeat - 1)
...
def symmetrize(sele="not symmetrized_*", alignsele=None, chains=[], delete=True):
...
def showcxaxis(sele, nfold=None, chains=[], length=30, col=(1, 1, 1), lbl="Cx Axis"):
...
def rechain(sel, nres):
    ...
    for i in range(ntot / nres):
        cmd.alter("resi %i-%i" % (nres * i + 1, nres * (i + 1)),
                  "chain='%s'" % chains[i])
...
def gen_helical_alignments(sele1, sele2, pref="HALN"):
    ...
    chunks1 = ["chain A and resi " + str(h)[1:-1].replace(', ', '+')
               for h in get_fixed_size_contigs("chain A and %s and ss H" % sele1)]
    chunks2 = ["chain Z and resi " + str(h)[1:-1].replace(', ', '+')
    ...
    for i, hsel1 in enumerate(chunks1):
        ...
        for j, hsel2 in enumerate(chunks2):
            ...
            # now align them
            cmd.align(name2 + " and chain Z and " + hsel2,
                      name1 + " and chain A and " + hsel1)
...
def alignsym(sel="all", arch="I32", ax1=Vec(0, 0, 1), ax2=Vec(0.356825, 0.000002, 0.934171)):
    if arch == "T32":
        tgt1 = Vec(1.00000000000000, 1.00000000000000,
                   1.00000000000000).normalized()
        tgt2 = Vec(1.00000000000000, 0.00000000000000,
                   0.00000000000000).normalized()
    if arch == "T33":
        tgt1 = Vec(1.00000000000000, 1.00000000000000,
                   1.00000000000000).normalized()
        tgt2 = Vec(1.00000000000000, 1.00000000000000, -
                   1.00000000000000).normalized()
    if arch == "O32":
        tgt1 = Vec(1.00000000000000, 1.00000000000000,
                   1.00000000000000).normalized()
        tgt2 = Vec(1.00000000000000, 1.00000000000000,
                   0.00000000000000).normalized()
    if arch == "O42":
        tgt1 = Vec(1.00000000000000, 0.00000000000000,
                   0.00000000000000).normalized()
        tgt2 = Vec(1.00000000000000, 1.00000000000000,
                   0.00000000000000).normalized()
    if arch == "O43":
        ...
        tgt2 = Vec(1.00000000000000, 1.00000000000000,
                   1.00000000000000).normalized()
    if arch == "I32":
        tgt1 = Vec(0.93417235896272, 0.00000000000000,
                   0.35682208977309).normalized()
        tgt2 = Vec(1.00000000000000, 0.00000000000000,
                   0.00000000000000).normalized()
    if arch == "I52":
        tgt1 = Vec(0.85065080835204, 0.52573111211914,
                   0.00000000000000).normalized()
        tgt2 = Vec(1.00000000000000, 0.00000000000000,
                   0.00000000000000).normalized()
    if arch == "I53":
        tgt1 = Vec(0.85065080835204, 0.52573111211914,
                   0.00000000000000).normalized()
        tgt2 = Vec(0.93417235896272, 0.00000000000000,
                   0.35682208977309).normalized()
