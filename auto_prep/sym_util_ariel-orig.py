...
def find_symelems(sele_or_xforms="all",verbose=False):
	...
	for c,x in xforms.items():
		...
		if c[0]==c[1]: continue
		dis = x.t.length()
		#print " in find_symelems printing c, x, dis: ",c,x,dis 
		if dis > 5.0: continue
	...
	if verbose:
		for se1,se2 in filter( lambda t: t[0]<t[1], product(symelems,symelems) ):
			if se1[0]==se2[0]:
				print "se1: ", se1
				print "se2: ", se2
				print "degrees", symelemdis(se1,se2)
				print
...
def guessdxaxes(sele="all",verbose=False):
	...
	assert nfold % 2 is 0
	nfold /= 2
	symelems, maxrms, angerr, axiserr = find_symelems(sele,verbose=True)
	print "                  symelems:",symelems # symelems[0][0], symelems[1][0]
	print "\n symelems[0][0]: ",symelems[0][0]
	print "\n symelems[1][0]: ",symelems[1][0]
	print "\n symelems[][0]: ",symelems[2][0]
	assert len(symelems) > 1
	assert symelems[0][0] == float(nfold)
	assert symelems[-1][0] == float(2)
	for e in symelems:
		assert e[0] in (nfold, 2)
	assert symelems[-1][0] == float(2)
