...
parser.add_option("-p", dest="pymol_scripts_dir", default="%s" %(os.getenv("HOME")+"/pymolscripts/"), help="Location of Will's util.py pymol module") 
...
if options.add_to_database:
	database_file = open(os.getenv("HOME")+"/bakerlab/crystal_design/dihedral_scaffolds/dihedral_scaffolds.dat","r")
...
#we don't think we are going to need this, so we aren't changing it ^
	...
	if options.add_to_database:
		output_pdb_path = os.getenv("HOME")+"/bakerlab/crystal_design/dihedral_scaffolds/"+input_pdbname[1:3]+"/"
...
if options.add_to_database:
	...
	database_file = open(os.getenv("HOME")+"/bakerlab/crystal_design/dihedral_scaffolds/dihedral_scaffolds.dat","w")
