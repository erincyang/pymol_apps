...
def getres(sele,withchain=True):
	try:
		...
		r.sort()
		if withchain: return r
		else: return [x[1] for x in r]
	except:
		return []
...
def showvec(c,lbl=''):
	...
	if not lbl:
		...
		lbl = "vec%i"%numvec
		numvec += 1
	mycgo = [cgo.COLOR, 1.0, 1.0, 1.0, cgo.SPHERE, c.x, c.y, c.z, 1.0] ## white sphere with 3A radius
...
def showvecfrompoint(a, c, col=(1,1,1), lbl=''):
	...
	cmd.load_cgo([cgo.COLOR, col[0],col[1],col[2],
				  cgo.SPHERE,   c.x,       c.y,       c.z,    0.5,
				  cgo.CYLINDER, c.x,       c.y,       c.z,
							c.x + a.x, c.y + a.y, c.z + a.z, 0.05,
