...
def objname(objpath):
    ...
    return re.sub( r'(\.pdb|\.pdb.gz|\.ent.gz)$', '', path.basename(objpath))
...
class PDBDirCycler(EnableCycler):
    ...
    def __init__(self,target_dir='.'):
        ...
        self.pdbs = [x for x in glob(path.join(target_dir, "*.pdb")) if x[0]!='.' and x[-3:]!='csv']

        print('load1', self.pdbs)
...
class PDBDirCyclerLite(LoadDeleteCycler):
    ...
    def __init__(self,target_dir='.'):
        ...
        pdbs_in_dir = sorted([f for f in os.listdir(target_dir) if (re.search('.cif.*$',f) or re.search('pdb*',f)) and f[0]!='.' and f[-3:]!='csv' and f[-3:]!='.sc'] )

        print(f'Cycler found {len(pdbs_in_dir)} pdbs')

        assert len(pdbs_in_dir) >= 0

        self.pdbs = pdbs_in_dir
...
def spawnPDBDirCycler(target_dir='.',lite=False):
    ...
    cmd.set_key('CTRL-B',prev_pdb)

    cmd.set_key('CTRL-D',next_pdb)
