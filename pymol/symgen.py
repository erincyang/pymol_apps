...
class SymElem(object):
    ...
    def __init__(self,
        ...
        if self.kind.startswith("C"):
            ...
            self.frames = [
                Xform(
                    Mat(Vec(1, 0, 0), Vec(0, 1, 0), Vec(0, 0, 1)), Vec(
                        0, 0, 0)),
                Xform(
                    Mat(Vec(0, 0, 1), Vec(1, 0, 0), Vec(0, 1, 0)), Vec(
                        0, 0, 0)),
                Xform(
                    Mat(Vec(0, 0, 1), Vec(-1, 0, 0), Vec(0, -1, 0)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(0, 0, -1), Vec(1, 0, 0), Vec(0, -1, 0)),
                    Vec(0, 0, 0)),
                Xform(
                    ...
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(0, 1, 0), Vec(0, 0, 1), Vec(1, 0, 0)), Vec(
                        0, 0, 0)),
                Xform(
                    Mat(Vec(0, 1, 0), Vec(0, 0, -1), Vec(-1, 0, 0)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(0, -1, 0), Vec(0, 0, 1), Vec(-1, 0, 0)),
                    Vec(0, 0, 0)),
                Xform(
                    ...
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(1, 0, 0), Vec(0, -1, 0), Vec(0, -0, -1)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(-1, 0, 0), Vec(0, 1, 0), Vec(0, 0, -1)),
                    Vec(0, 0, 0)),
            ...
            self.frames = [
                Xform(
                    ...
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(+0, +1, +0), Vec(+1, +0, -0), Vec(-0, +0, -1)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(+0, -0, +1), Vec(+1, +0, -0), Vec(-0, +1, +0)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(+1, +0, -0), Vec(+0, -0, +1), Vec(+0, -1, -0)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(-0, +0, -1), Vec(+0, +1, +0), Vec(+1, -0, -0)),
                    Vec(0, 0, 0)),
                Xform(
                    Mat(Vec(-0, +1, +0), Vec(+0, -0, +1), Vec(+1, +0, -0)),
                    Vec(0, 0, 0)),
        ...
        if self.kind[0] in "CD":
            if not col:
                if self.kind == "C2":
                    ...
                    raise NotImplementedError("unknown symelem kind " +
                                              self.kind)
            ...
            for f in self.frames:
                ...
                if not c3a in seen3:
                    CGO.extend(
                        cgo_cyl(cen + c3a, cen + c3b, radius, col=(0, 1, 0)))
                    seen3.append(c3a)
                    seen3.append(c3b)
        elif self.kind == "O":
            cen = x * self.cen
            ...
            for f in self.frames:
                ...
                if not c2b in seen2:
                    CGO.extend(
                        cgo_cyl(cen + c2a, cen + c2b, radius, col=(1, 0, 0)))
                    seen2.append(c2a)
                    seen2.append(c2b)
                if not c3a in seen3:
                    CGO.extend(
                ...
                if not c4a in seen4:
                    CGO.extend(
                        cgo_cyl(cen + c4a, cen + c4b, radius, col=(0, 0, 1)))
                    seen4.append(c4a)
                    seen4.append(c4b)
        if showshape:
            if self.kind == "C2":
                axs = x.R * self.axis
                cen = x * self.cen
                p1 = x.R * projperp(self.axis, Vec(7, 3,
                                                   1)).normalized() * 30.0
                p2 = RAD(axs, 180.0) * p1
                p3 = x.R * projperp(self.axis, Vec(1, 3,
                                                   7)).normalized() * 30.0
                p4 = RAD(axs, 180.0) * p3
                p1 = cen + p1
                p2 = cen + p2
...
class SymTrieNode(object):
    ...
    def __str__(self):
        return "elem %2i frame %2i depth %2i nchild %2i" % (
            self.ielem, self.iframe, self.depth, len(self.children))
...
def generate_sym_trie_recurse(generators, depth, opts, body, heads, newheads,
                              igen):
    if depth < 1:
        return
    print("GEN SYM TRIE", "depth", depth, "igen", igen, "heads", len(heads),
          "newheads", len(newheads))
    newnewheads = []
    ...
    for iframe, frame in enumerate(elem.frames):
        ...
        for head, xpos in itertools.chain(newheads, heads):
            ...
            seenit = False
            for seennode, seenframe in itertools.chain(newnewheads, newheads,
                                                       heads, body):
    ...
    if depth > 1:  # and newheads:
        generate_sym_trie_recurse(generators, depth - 1, opts, body, heads,
                                  newheads, (igen + 1) % len(generators))
...
def generate_sym_trie(generators, depth=10, opts=None):
    ...
    body = list()
    newheads = list()
    generate_sym_trie_recurse(generators, depth, opts, body, heads, newheads,
                              0)
...
def make_d3oct(d3, cage, cage_trimer_chain="A", depth=4, maxrad=9e9):
    ...
    makesym(
        G,
        sele="((" + d3 + ") and ((chain A+B) and name CA))",
        depth=depth,
        maxrad=maxrad)
...
class BuildCGO(object):
    ...
    def __init__(self,
        ...
        self.origin = origin
        self.bbox = bbox
        self.bbox[0].x, self.bbox[1].x = min(self.bbox[0].x,
                                             self.bbox[1].x), max(
                                                 self.bbox[0].x,
                                                 self.bbox[1].x)
        self.bbox[0].y, self.bbox[1].y = min(self.bbox[0].y,
                                             self.bbox[1].y), max(
                                                 self.bbox[0].y,
                                                 self.bbox[1].y)
        self.bbox[0].z, self.bbox[1].z = min(self.bbox[0].z,
                                             self.bbox[1].z), max(
                                                 self.bbox[0].z,
                                                 self.bbox[1].z)
        ...
        if self.showelems:
            for elem in node.generators:
                if self.bounds_check(x * elem.cen):
                    mergeargs = dict(
                        list(kwargs.items()) + list(self.kwargs.items()))
...
class ComponentCenterVisitor(object):
    ...
    def __call__(self, sym_trie_node, xform, parentxform, **kwargs):
        assert list(self.priCCtoCCframes.keys()) == list(
            self.priCCtoCClist.keys())
    ...
    def check_jumps(self):
        ...
        for k in list(jset.keys()):
            xyz = sorted((abs(round(k.x, 5)), abs(round(k.y, 5)),
                          abs(round(k.z, 5))))
            # print xyz
    ...
    def show(self,
             component_pos=(Vec(3, 11, 9), Vec(11, 3, 9), Vec(11, 9, 3),
                            Vec(9, 3, 11)),
             showframes=True,
             **kwargs):
        self.sanity_check()
        ...
        for ipn, itms in enumerate(self.priCCtoCClist.items()):
            ...
            for n in CClist:
                ...
                if n in list(self.parentmap.keys()
                             ) and self.parentmap[n] and self.showlinks:
                    CGO.extend(
                        cgo_cyl_arrow(
                ...
                if showframes and ipn < len(component_pos):
                    for stn in self.priCCtoCCframes[pn][n]:
                        ...
                        CGO.extend(
                            cgo_sphere(cnx, r=1.7, col=self.colors[ipn]))
                        CGO.extend(
                            cgo_sphere(cny, r=1.2, col=self.colors[ipn]))
    ...
    def make_symdef(self, **kwargs):
        ...
        for ip, val in enumerate(self.priCCtoCCframes.items()):
            ...
            for icc, val2 in enumerate(CCframes.items()):
                ...
                CC, STNs = val2
                assert len(STNs) > 0
                PCC = None if not CC in list(
                    self.parentmap.keys()) else self.parentmap[CC]
                if PCC:
                ...
                if PCC:
                    Sxyz += (XYZ_TEMPLATE % (
                        PCCDofBegName, DIR.x, DIR.y, DIR.z, DIR2.x, DIR2.y,
                        DIR2.z, PCC.x * scale, PCC.y * scale, PCC.z * scale))
                if PCC:
                    Sxyz += (XYZ_TEMPLATE % (
                        PCCDofEndName, DIR.x, DIR.y, DIR.z, DIR2.x, DIR2.y,
                        DIR2.z, CC.x * scale, CC.y * scale, CC.z * scale))
                if True:
                    Sxyz += (XYZ_TEMPLATE %
                             (CCDofBegName, ELEMDIR.x, ELEMDIR.y, ELEMDIR.z,
                              ELEMDIR2.x, ELEMDIR2.y, ELEMDIR2.z, CC.x * scale,
                              CC.y * scale, CC.z * scale))
                if True:
                    Sxyz += (XYZ_TEMPLATE %
                             (CCDofEndName, ELEMDIR.x, ELEMDIR.y, ELEMDIR.z,
                              ELEMDIR2.x, ELEMDIR2.y, ELEMDIR2.z, CC.x * scale,
                              CC.y * scale, CC.z * scale))
                if PCC:
                ...
                for isub, stn in enumerate(STNs):
                    ...
                    Sxyz += (XYZ_TEMPLATE %
                             (SUBName, SX.x, SX.y, SX.z, SY.x, SY.y, SY.z,
                              SO.x * scale, SO.y * scale, SO.z * scale))
        ...
        if 'generic_names' in kwargs and kwargs['generic_names']:
            ...
            for icomp, dofs in enumerate(compdofjumps):
                if not dofs:
                    continue
                s = s.replace("JUMP__%s__to__%s" % dofs[0],
                              "COMP_DOF_%i" % (icomp + 1))
