...
class Mat(object):
   ...
   def row(m,i):
      ...
      if   i == 0: return Vec(m.xx,m.xy,m.xz)
      elif i == 1: return Vec(m.yx,m.yy,m.yz)
      elif i == 2: return Vec(m.zx,m.zy,m.zz)
      else: assert 0 <= i and i <= 2
   def col(m,i):
      assert type(i) is type(1)
      if   i == 0: return Vec(m.xx,m.yx,m.zx)
      elif i == 1: return Vec(m.xy,m.yy,m.zy)
      elif i == 2: return Vec(m.xz,m.yz,m.zz)
      else: assert 0 <= i and i <= 2
...
def rot_by_matrix(sel,R,cen=Vec(0,0,0)):
    m = cmd.get_model(sel)
...
#	b = cmd.get_model(alignsele+" and chain "+chains[1]+" and name CA").atom
	a = cmd.get_model(alignsele+" and chain "+chains[0]).atom
	b = cmd.get_model(alignsele+" and chain "+chains[1]).atom
...
def aligntrimer(sele,alignsele=None,tgtaxis=Vec(0,0,1),chains=["A","G","I"]):
	if alignsele is None: alignsele = sele
	cmd.remove(sele+" and resn HOH")
...
def alignsym(sele,dchains,tchains,dtgtaxis,ttgtaxis):
	daxis = dimeraxis(sele,sele,dchains)
	alignaxis(sele,dtgtaxis,daxis,Vec(0,0,0))
	taxis = dimeraxis(sele,sele,tchains)
	print( daxis,taxis)
	taxisd = projperp(daxis,taxis)
	ttgtaxisd = projperp(daxis,ttgtaxis)
	alignaxis(sele,ttgtaxisd,taxisd,Vec(0,0,0))
	print( daxis,taxis)


def aligntrimersindir(dir):
	for fn in glob.glob(dir+"/*.3mer"):
		cmd.delete("all")
		cmd.load(fn)
...
def aligntrimerz(sele,alignsele=None):
    ...
    cmd.remove(sele+" and resn HOH")
    trans(sele,-com(alignsele))
    a = cmd.get_model(alignsele+" and chain X and name CA").atom
    b = cmd.get_model(alignsele+" and chain Y and name CA").atom
    c = cmd.get_model(alignsele+" and chain Z and name CA").atom
    #print "subunit lengths:",len(a),len(b),len(c)
    if len(a) != len(b) or len(a) != len(c) or len(a) == 0:
        print( "%s\t%s\t" % (len(a),len(b)))
        print( "ERROR on %s: subunits are not the same size!"%alignsele)
        return False
    axis = Vec(0,0,0)
    for i in range(len(a)):
...
def alignpentamerz(sele,alignsele=None):
	if alignsele is None: alignsele = sele
	cmd.remove(sele+" and resn HOH")
	trans(sele,-com(alignsele))
	...
	d = cmd.get_model(alignsele+" and chain D and name CA").atom
	e = cmd.get_model(alignsele+" and chain E and name CA").atom
	print( "subunit lengths:",len(a),len(b),len(c),len(d),len(e))
	if len(a) != len(b) or len(a) != len(c) or len(a) == 0:
		print( "%s\t%s\t" % (len(a),len(b)))
		print( "ERROR on %s: subunits are not the same size!"%alignsele)
		return False
...
def alignhexamerz(sele,alignsele=None):
	if alignsele is None: alignsele = sele
	cmd.remove(sele+" and resn HOH")
	trans(sele,-com(alignsele))
	a = cmd.get_model(alignsele+" and chain A and name CA").atom
	...
	d = cmd.get_model(alignsele+" and chain D and name CA").atom
	e = cmd.get_model(alignsele+" and chain E and name CA").atom
	f = cmd.get_model(alignsele+" and chain F and name CA").atom
	# print "subunit lengths:",len(a),len(b),len(c)
	if len(a) != len(b) or len(a) != len(c) or len(a) == 0:
		print( "%s %s %s %s %s %s " % (len(a),len(b),len(c),len(d),len(e),len(f)))
		print( "ERROR on %s: subunits are not the same size!"%alignsele)
		return False
	axis = Vec(0,0,0)
	for i in range(len(a)):
		axis1 = ( Vec(a[i].coord)+Vec(b[i].coord)+Vec(c[i].coord)+Vec(d[i].coord)+Vec(e[i].coord)+Vec(f[i].coord) ).normalized()
		if axis.length() > 0.0001 and axis.dot(axis1) < 0:
			axis1 *= -1
		axis += axis1		
		# print axis1
...
def alignpentamersindir(dir):
	for fn in glob.glob(dir+"/*.5mer"):
		cmd.delete("all")
		cmd.load(fn)
		if alignpentamerz(fn[len(fn)-9:len(fn)-5]):
			cmd.save(fn+"_alignz.pdb","chain A")
			print( "SUCCESS on",fn,fn[len(fn)-9:len(fn)-5])
		else:
			print( "ERROR on",fn)
...
def aligntetramerz(sele,alignsele=None):
	if alignsele is None: alignsele = sele
	cmd.remove(sele+" and resn HOH")
	trans(sele,-com(alignsele))
	a = cmd.get_model(alignsele+" and chain A and name CA").atom
...
def aligntetramersindir(dir):
	for fn in glob.glob(dir+"/*.4mer"):
		cmd.delete("all")
		cmd.load(fn)
		if aligntetramerz(fn[len(fn)-9:len(fn)-5]):
			cmd.save(fn+"_alignz.pdb","chain A")
			print( "SUCCESS on",fn,fn[len(fn)-9:len(fn)-5])
		else:
			print( "ERROR on",fn)

# FROM PACKING MEASURE UTIL
...
def expandRadii(delta=1.0, sel='all'):
	for a in cmd.get_model(sel).atom:	
		...
		cmd.alter(f"index {a.index}",f'vdw={r}')
...
def calc_rmsd(sel1, sel2):
	m1 = cmd.get_model(sel1)
	m2 = cmd.get_model(sel2)
	assert len(m1.atom) == len(m2.atom)
	v1 = [Vec(x.coord) for x in m1.atom]
	v2 = [Vec(x.coord) for x in m2.atom]
	rmsd=0
	for a,b in zip(v1,v2): rmsd += (a-b).length()**2
	print( math.sqrt(rmsd/len(m1.atom)))
