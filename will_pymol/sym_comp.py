...
def procCdat(N=3, lfile=None, biod="/data/biounit", outd=None):
    if lfile is None:
        lfile = os.path.expanduser(
            "~/Dropbox/project/sym_comp/meta/C%i.dat" % N)
    if outd is None:
        ...
        try:
            ...
            if cmd.select('all', state=N) != 0:
                ...
                if len(cc) < N:
                    ...
                    continue
                for i in range(1, N + 1):
                    cmd.create("sub%i" % i, "m and chain %s" %
                               (cc[-i][1]), 1, 1)
            for i in range(1, N + 1):
                if iscontig("sub%i" % i):
            ...
            for i in range(1, N + 1):
                trans("sub%i" % i, -cm)
            a = [cmd.get_model("sub%i and name CA" %
                               i).atom for i in range(1, N + 1)]
            axis = xyz.Vec(0, 0, 0)
            ...
            axis.normalize()
            for i in range(1, N + 1):
                alignaxis("sub%i" % i, xyz.Vec(
                    0, 0, 1), axis, xyz.Vec(0, 0, 0))
...
def make_cryst1_23(fn, a2in, i2, a3in, i3):
    ...
    olda2 = c2axis(Dsel, chains=('A', 'D'))
    olda3 = c3axis(Tsel)
    axis_angle_error = abs(a2in.lineangle_degrees(
        a3in) - olda2.lineangle_degrees(olda3))
