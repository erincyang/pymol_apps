    ...
    def __init__(self, kind, axis=Vec(0, 0, 1), cen=Vec(0, 0, 0), axis2=Vec(1, 0, 0), col=None, input_xform=None):
        ...
        if self.kind.startswith("C"):
            ...
            self.frames = [
                ...
                Xform(Mat(Vec(0, 1, 0), Vec(0, 0, -1),
                          Vec(-1, 0, 0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(0, -1, 0), Vec(0, 0, 1),
                          Vec(-1, 0, 0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(0, -1, 0), Vec(0, 0, -1),
                          Vec(1, 0, 0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(1, 0, 0), Vec(0, -1, 0),
                          Vec(0, -0, -1)), Vec(0, 0, 0)),
                Xform(Mat(Vec(-1, 0, 0), Vec(0, 1, 0),
                          Vec(0, 0, -1)), Vec(0, 0, 0)),
                Xform(Mat(Vec(-1, 0, 0), Vec(0, -1, 0), Vec(0, 0, 1)), Vec(0, 0, 0))
            ...
            self.frames = [
                Xform(Mat(Vec(+1, +0, -0), Vec(+0, +1, +0),
                          Vec(+0, -0, +1)), Vec(0, 0, 0)),
                Xform(Mat(Vec(+0, +1, +0), Vec(+1, +0, -0),
                          Vec(-0, +0, -1)), Vec(0, 0, 0)),
                Xform(Mat(Vec(+0, -0, +1), Vec(+1, +0, -0),
                          Vec(-0, +1, +0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(+1, +0, -0), Vec(+0, -0, +1),
                          Vec(+0, -1, -0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(-0, +0, -1), Vec(+0, +1, +0),
                          Vec(+1, -0, -0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(-0, +1, +0), Vec(+0, -0, +1),
                          Vec(+1, +0, -0)), Vec(0, 0, 0)),
                Xform(Mat(Vec(+0, +1, +0), Vec(-0, +0, -1),
        ...
        if self.kind[0] in "CD":
            if not col:
                if self.kind == "C2":
                    ...
                    raise NotImplementedError(
                        "unknown symelem kind " + self.kind)
            ...
            for f in self.frames:
                ...
                if not c2b in seen2:
                    ...
                    seen2.append(c2b)
                if not c3a in seen3:
                    CGO.extend(cgo_cyl(cen + c3a, cen +
                                       c3b, radius, col=(0, 1, 0)))
                    seen3.append(c3a)
                    seen3.append(c3b)
        elif self.kind == "O":
            cen = x * self.cen
            ...
            for f in self.frames:
                ...
                c4b.round0()
                if not c2b in seen2:
                    CGO.extend(cgo_cyl(cen + c2a, cen +
                                       c2b, radius, col=(1, 0, 0)))
                    seen2.append(c2a)
                    seen2.append(c2b)
                if not c3a in seen3:
                    CGO.extend(cgo_cyl(cen + c3a, cen +
                    ...
                    seen3.append(c3b)
                if not c4a in seen4:
                    CGO.extend(cgo_cyl(cen + c4a, cen +
                                       c4b, radius, col=(0, 0, 1)))
                    seen4.append(c4a)
                    seen4.append(c4b)
        if showshape:
            if self.kind == "C2":
                axs = x.R * self.axis
                cen = x * self.cen
                p1 = x.R * projperp(self.axis, Vec(7, 3, 1)
                                    ).normalized() * 30.0
                p2 = RAD(axs, 180.0) * p1
                p3 = x.R * projperp(self.axis, Vec(1, 3, 7)
                                    ).normalized() * 30.0
                p4 = RAD(axs, 180.0) * p3
                p1 = cen + p1
                p2 = cen + p2
                ...
                # print p1,p2,p3,p4
                CGO.extend([
                    pymol.cgo.BEGIN, pymol.cgo.TRIANGLES, pymol.cgo.COLOR, col[
                        0], col[1], col[2], pymol.cgo.ALPHA, 1,
                    pymol.cgo.NORMAL, axs.x, axs.y, axs.z, pymol.cgo.VERTEX,  p1.x +
                    axs.x / 10.0, p1.y + axs.y / 10.0, p1.z + axs.z / 10.0,
                    pymol.cgo.NORMAL, axs.x, axs.y, axs.z, pymol.cgo.VERTEX,  p2.x +
                    axs.x / 10.0, p2.y + axs.y / 10.0, p2.z + axs.z / 10.0,
                    pymol.cgo.NORMAL, axs.x, axs.y, axs.z, pymol.cgo.VERTEX,  p3.x +
                    axs.x / 10.0, p3.y + axs.y / 10.0, p3.z + axs.z / 10.0,
                    pymol.cgo.NORMAL, axs.x, axs.y, axs.z, pymol.cgo.VERTEX,  p1.x +
                    axs.x / 10.0, p1.y + axs.y / 10.0, p1.z + axs.z / 10.0,
                    pymol.cgo.NORMAL, axs.x, axs.y, axs.z, pymol.cgo.VERTEX,  p2.x +
                    axs.x / 10.0, p2.y + axs.y / 10.0, p2.z + axs.z / 10.0,
                    pymol.cgo.NORMAL, axs.x, axs.y, axs.z, pymol.cgo.VERTEX,  p4.x +
                    axs.x / 10.0, p4.y + axs.y / 10.0, p4.z + axs.z / 10.0,
...
def generate_sym_trie_recurse(generators, depth, opts, body, heads, newheads, igen):
    ...
    for iframe, frame in enumerate(elem.frames):
        ...
        for head, xpos in itertools.chain(newheads, heads):
            ...
            if not seenit:
                newhead = SymTrieNode(
                    generators, ielem, iframe, head.depth + 1, position=candidate)
    ...
    if depth > 1:  # and newheads:
        generate_sym_trie_recurse(
            generators, depth - 1, opts, body, heads, newheads, (igen + 1) % len(generators))
...
def generate_sym_trie(generators, depth=10, opts=None):
    ...
    body = list()
    newheads = list()
    generate_sym_trie_recurse(generators, depth, opts,
                              body, heads, newheads, 0)
...
def makesym(frames0, sele="all", newobj="MAKESYM", depth=None, maxrad=9e9, n=9e9):
    ...
    for i, x in enumerate(frames):
        ...
        for j, c in enumerate(selechains):
            cmd.alter(tmpname + " and chain " + c, "chain='%s'" %
                      ROSETTA_CHAINS[len(selechains) * i + j])
        xform(tmpname, x)
...
def make_d3oct(d3, cage, cage_trimer_chain="A", depth=4, maxrad=9e9):
    ...
    G = [RAD(Ux, 180), RAD(Uz, 120), RAD(x * Vec(1, 0, 0), 90, Vec(0,
                                                                   0, zcagecen)), RAD(x * Vec(1, 1, 0), 180, Vec(0, 0, zcagecen)), ]
    makesym(G, sele="((" + d3 + ") and ((chain A+B) and name CA))",
            depth=depth, maxrad=maxrad)
...
def make_d3tet(d3, cage, cage_trimer_chain="A", depth=4, maxrad=9e9):
    ...
    # print x * Vec(1,1,1), x*Vec(1,-1,0)
    # RAD(Ux,180), RAD(Uy,120),
    G = [RAD(Ux, 180), RAD(Uz, 120), RAD(
        x * Vec(1, 0, 0), 180, Vec(0, 0, zcagecen)), ]
    makesym(G, sele="((" + d3 + ") and ((chain A+B) and name CA))",
            depth=depth, maxrad=maxrad)
    cmd.show("sph", "MAKESYM")
    # cmd.disable("all")
...
def cgo_cyl_arrow(c1, c2, r, col=(1, 1, 1), col2=None, arrowlen=4.0):
    ...
    perp = projperp(dirn, Vec(0.2340790923, 0.96794275,
                              0.52037438472304783)).normalized()
...
class BuildCGO(object):
    ...
    def __init__(self, nodes, maxrad=9e9, origin=Vec(0, 0, 0), bbox=(Vec(-9e9, -9e9, -9e9), Vec(9e9, 9e9, 9e9)),
        ...
        self.origin = origin
        self.bbox = bbox
        self.bbox[0].x, self.bbox[1].x = min(
            self.bbox[0].x, self.bbox[1].x), max(self.bbox[0].x, self.bbox[1].x)
        self.bbox[0].y, self.bbox[1].y = min(
            self.bbox[0].y, self.bbox[1].y), max(self.bbox[0].y, self.bbox[1].y)
        self.bbox[0].z, self.bbox[1].z = min(
            self.bbox[0].z, self.bbox[1].z), max(self.bbox[0].z, self.bbox[1].z)
    ...
    def __call__(self, node, **kwargs):
        ...
        for icen, cen in enumerate(self.nodes):
            ...
            if self.bounds_check(xcen):
                self.add_sphere(x * (cen + Vec(0, 0, 0)), 2.0, text="%s%i" %
                                ("ABCD"[icen], node.depth), icol=icen)
                self.add_sphere(x * (cen + Vec(2, 0, 0)), 2.0, text="%s%i" %
                                ("ABCD"[icen], node.depth), icol=icen)
                self.add_sphere(x * (cen + Vec(0, 2, 0)), 2.0, text="%s%i" %
                                ("ABCD"[icen], node.depth), icol=icen)
            pcen = xcen
    ...
    def add_segment(self, c1, c2, icol):
        ...
        if c1.distance(c2) < 1.0:
            return
        self.CGO.extend(cgo_cyl_arrow(
            c1, c2, r=0.5, col=self.colors[max(0, icol - 1)], col2=self.colors[icol]))
...
class VecDict(object):
    ...
    def items(self):
        return list(zip(self.keys_, self.values_))
...
class ComponentCenterVisitor(object):
    ...
    def check_jumps(self):
        ...
        for k in list(jset.keys()):
            xyz = sorted((abs(round(k.x, 5)), abs(
                round(k.y, 5)), abs(round(k.z, 5))))
            # print xyz
        ...
        for ipn, itms in enumerate(self.priCCtoCClist.items()):
            ...
            for n in CClist:
                ...
                if n in list(self.parentmap.keys()) and self.parentmap[n] and self.showlinks:
                    CGO.extend(cgo_cyl_arrow(
                        self.parentmap[n], n, r=0.8, col=self.colors[ipn]))
                if showframes and ipn < len(component_pos):
                    for stn in self.priCCtoCCframes[pn][n]:
                        ...
                        CGO.extend(cgo_sphere(
                            cnx, r=1.7, col=self.colors[ipn]))
                        CGO.extend(cgo_sphere(
                            cny, r=1.2, col=self.colors[ipn]))
                        if self.showlinks or True:
                            CGO.extend(cgo_cyl_arrow(
                                n, cn, r=0.3, col=self.colors[ipn], arrowlen=2.0))
        v = cmd.get_view()
        cmd.delete(self.label)
    ...
    def make_symdef(self, **kwargs):
        ...
        for ip, val in enumerate(self.priCCtoCCframes.items()):
            ...
            for icc, val2 in enumerate(CCframes.items()):
                ...
                CC, STNs = val2
                assert len(STNs) > 0
                PCC = None if not CC in list(self.parentmap.keys(
                )) else self.parentmap[CC]
                if PCC:
                ...
                if PCC:
                    Sxyz += (XYZ_TEMPLATE % (PCCDofBegName, DIR.x, DIR.y, DIR.z, DIR2.x,
                                             DIR2.y, DIR2.z, PCC.x * scale, PCC.y * scale, PCC.z * scale))
                if PCC:
                    Sxyz += (XYZ_TEMPLATE % (PCCDofEndName, DIR.x, DIR.y, DIR.z, DIR2.x,
                                             DIR2.y, DIR2.z,  CC.x * scale, CC.y * scale, CC.z * scale))
                if True:
                    Sxyz += (XYZ_TEMPLATE % (CCDofBegName, ELEMDIR.x, ELEMDIR.y, ELEMDIR.z,
                                             ELEMDIR2.x, ELEMDIR2.y, ELEMDIR2.z, CC.x * scale, CC.y * scale, CC.z * scale))
                if True:
                    Sxyz += (XYZ_TEMPLATE % (CCDofEndName, ELEMDIR.x, ELEMDIR.y, ELEMDIR.z,
                                             ELEMDIR2.x, ELEMDIR2.y, ELEMDIR2.z, CC.x * scale, CC.y * scale, CC.z * scale))
                if PCC:
                ...
                for isub, stn in enumerate(STNs):
                    ...
                    Sxyz += (XYZ_TEMPLATE % (SUBName, SX.x, SX.y, SX.z, SY.x,
                                             SY.y, SY.z, SO.x * scale, SO.y * scale, SO.z * scale))
        ...
        if 'generic_names' in kwargs and kwargs['generic_names']:
            ...
            for icomp, dofs in enumerate(compdofjumps):
                if not dofs:
                    continue
                s = s.replace("JUMP__%s__to__%s" %
                              dofs[0], "COMP_DOF_%i" % (icomp + 1))
...
def Sheet_P6_C3_C2(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3( depth=4, cell=100, symdef_scale=0.000001, generic_names=1 )
    G = [ SymElem( 'C3', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), #axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec( 0,-0.5,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
        ]
    test_xtal(G,cell,tag='Sheet_P6_C3_C2',**kwargs)

def Sheet_P6_C6_C2(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'C6', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), #axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec( 0,1,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
        ]
    test_xtal(G,cell,tag='Sheet_P6_C6_C2',**kwargs)

def Sheet_P622_D3_C2(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    ...
    test_xtal(G,cell,tag='Sheet_P622_D3_C2',**kwargs)

def Sheet_P622_C6_C2(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'C6', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec( 0,1,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
    ...
    test_xtal(G,cell,tag='Sheet_P622_C6_C2',**kwargs)

def Sheet_P622_D2_C2(cell=100,**kwargs): #### NOT WORKING
    G = [ SymElem( 'D2', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec( 0,1,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
...
def Sheet_P4_C4_C2(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'C4', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), #axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec( 1,0,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
        ]
    test_xtal(G,cell,tag='Sheet_P4_C4C2',**kwargs)

def Sheet_P4_C4_C4(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'C4', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), #axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C4', cen=cell*Vec( 1,1,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
        ]
    test_xtal(G,cell,tag='Sheet_P4_C4C4',**kwargs)

def Sheet_P422(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'D4', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), axis2=Vec(0.866025,0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec( 0,1,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
        ]
    test_xtal(G,cell,tag='Sheet_P422',**kwargs)

def Sheet_P312(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'D3', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), axis2=Vec(0.866025,-0.5,0) 
        ),
          SymElem( 'C2', cen=cell*Vec(0,1,0), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0)
    ...
    test_xtal(G,cell,tag='Sheet_P321',**kwargs)
    
def Sheet_P321(cell=100,**kwargs):
    #delete all ; run ~/pymolscripts/symgen2.py; test_P312_D3_D3(cell=100, depth=2)
    G = [ SymElem( 'C3', cen=cell*( Vec ( 0,0,0 ) ), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0) 
        ),
          SymElem( 'C3', cen=cell*Vec(0,1,-0.5), axis=Vec(0,0,1), #axis2=Vec(0.866025,-0.5,0)
            ),
          #SymElem( 'Z', cen=cell*Vec( 0,0,2 ) ),
        ]
    test_xtal(G,cell,tag='Sheet_P321',**kwargs)
...
def test_D4_2(cell=100,**kwargs): ###MAKE THIS SYMDEF
    # delete all; run ~/pymol/symgen.py; test_F432_2O()
    G = [
            SymElem( "C2",  cen=cell*Vec(1,1,0) ),   
            SymElem( "C2",  cen=cell*Vec(-1,1,0) ), 
            SymElem( "C2",  cen=cell*Vec(1,-1,0) ),
            SymElem( "C2",  cen=cell*Vec(-1,-1,0) ),
            
        ]
    test_xtal(G,cell,tag='test_D4_2',**kwargs)