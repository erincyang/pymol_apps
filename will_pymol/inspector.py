...
def setup_matdes(inspector):
    ...
    cmd.load("/work/sheffler/data/jacob_" +
             inspector.nat1[:2] + "/" + inspector.nat1 + ".pdb.gz")
    cmd.load("/work/sheffler/data/jacob_" +
             inspector.nat2[:2] + "/" + inspector.nat2 + ".pdb.gz")
    cmd.super(inspector.nat1, inspector.current + " and chain C")
    cmd.super(inspector.nat2, inspector.current + " and chain A")
    cmd.color("white", "(%s or %s) and elem C" %
              (inspector.nat1, inspector.nat2))
